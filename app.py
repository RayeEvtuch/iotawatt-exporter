...
POWER_USAGE_WATTS = Gauge('power_usage_watts', 'Total amount of watts being drawn', ['circuit'])
POWER_SUPPLY_VOLTS = Gauge('power_supply_volts', 'Total amount of volts being supplied', ['circuit'])
...
if __name__ == '__main__':
    ...
    while True:
        ...
        series_response = requests.get(url='http://iotawatt.local/query?show=series')
        series_data = json.loads(series_response.content)
        series = dict( [ (serie['name'],serie['unit']) for serie in series_data['series'] ] )
        ...
        # Query the IoTaWatt
        values_response = requests.get(url='http://iotawatt.local/query', params=query)
            ...
            if label in series.keys():
                ...
                if values_data['data'][0][i]:
                    if series[label] == 'Watts':
                        POWER_USAGE_WATTS.labels(label).set(values_data['data'][0][i])
                    elif series[label] == 'Volts':
                        POWER_SUPPLY_VOLTS.labels(label).set(values_data['data'][0][i])
                    else:
                        print("{} is not a valid unit for {}".format(series[label],label))
                else:
